...
class Scraper {
  constructor(metrics, octokit, graphql) {
    ...
    this.octokit.hook.after('request', async (response) => {
      this.scrapeRateLimitRest(response);
    });
  }
  ...
  scrapeRateLimitGraphql(response) {
  ...
  scrapeRepositories(repositories) {
    ...
    repositories.forEach((repository) => {
      ...
      this.scrapeRepositoryTrafficClones(repository);
      this.scrapeRepositoryTrafficTopPaths(repository);
      this.scrapeRepositoryTrafficTopReferrers(repository);
      this.scrapeRepositoryTrafficViews(repository);
    });
  }
  ...
  scrapeRepositoriesGraphQlDetails(repositories) {
    ...
    repositoryChunks.forEach((repositoryChunk) => {
      this.graphql(graphqlHelper.repositoryQuery(repositoryChunk))
        .then((response) => this.scrapeRateLimitGraphql(response))
        .then((response) => {
  ...
  scrapeRepositoryRestDetails(repository) {
    ...
    this.octokit.repos.get({ owner, repo })
      ...
      .catch((err) => console.error(`Failed to scrape repository ${repository} via REST: `, err));
  }

  scrapeRepositoryTrafficClones(repository) {
    const [owner, repo] = repository.split('/');

    this.octokit.repos.getClones({ owner, repo, per: 'day' })
      .then((response) => {
      ...
      .catch((err) => console.error(`Failed to scrape clone metrics for repository ${repository} via REST: `, err));
  }

  scrapeRepositoryTrafficTopPaths(repository) {
    const [owner, repo] = repository.split('/');
    this.octokit.repos.getTopPaths({ owner, repo })
      .then((response) => {
      ...
      .catch((err) => console.error(`Failed to scrape path metrics for repository ${repository} via REST: `, err));
  }

  scrapeRepositoryTrafficTopReferrers(repository) {
    const [owner, repo] = repository.split('/');
    this.octokit.repos.getTopReferrers({ owner, repo })
      .then((response) => {
      ...
      .catch((err) => console.error(`Failed to scrape referrer metrics for repository ${repository} via REST: `, err));
  }

  scrapeRepositoryTrafficViews(repository) {
    const [owner, repo] = repository.split('/');
    this.octokit.repos.getViews({ owner, repo })
      .then((response) => {
      ...
      .catch((err) => console.error(`Failed to scrape view metrics for repository ${repository} via REST: `, err));
  }

  scrapeRepositoryContributors(repository) {
    const [owner, repo] = repository.split('/');

    const options = this.octokit.repos.listContributors.endpoint.merge({ owner, repo });
    ...
    this.octokit.paginate(
      ...
      .then((repositoryContributors) => {
        this.metrics.githubRepoContributorsGauge.set({ repository }, repositoryContributors.length);
      })
      .catch((err) => console.error(`Failed to scrape contributors from repository ${repository}: `, err));
  }
}
