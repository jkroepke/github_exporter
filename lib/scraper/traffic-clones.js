...
class TrafficClones {
  ...
  scrapeRepositories (repositories) {
    repositories.forEach((repository) => {
      ...
      ghRestApi.repos.getClones({ owner, repo, per: 'week' })
        .then((response) => {
          this.metrics.githubRepoClonesAvgGauge.set(
          ...
          const currentWeek = response.data.clones
            .filter((metric) => currentWeekNumber === helpers.getWeekNumber(new Date(metric.timestamp)))

          this.metrics.githubRepoClonesGauge.set(
            { owner, repository, week: 'latest' }, (currentWeek.length > 0) ? currentWeek[0].count : 0
          )
          this.metrics.githubRepoClonesUniqueGauge.set(
            { owner, repository, week: 'latest' }, (currentWeek.length > 0) ? currentWeek[0].uniques : 0
          )
          ...
          response.data.clones
            ...
            .forEach((metric) => {
              ...
              this.metrics.githubRepoClonesGauge.set(
                { owner, repository, week: weekNumber }, metric.count
              )
              this.metrics.githubRepoClonesUniqueGauge.set(
                { owner, repository, week: weekNumber }, metric.uniques
              )
            })
        })
