...
class Summarize {
  ...
  scrapeRepositories(repositories) {
    ...
    repositoryChunks.forEach((repositoryChunk) => {
      graphql(Summarize.graphQlRepositoryQuery(repositoryChunk))
        .then((response) => {
          repositoryChunk.forEach(
            (repository) => {
              const repoLabel = `repo_${Summarize.transformRepositoryNameToGraphQlLabel(repository)}`;
  ...
  static transformRepositoryNameToGraphQlLabel(name) {
    return name.replace(/[-/.]/g, '_');
  }

  static graphQlRepositoryQuery(repositories) {
    const graphQlRepositoryQueryParts = repositories.map((repository) => `
    repo_${Summarize.transformRepositoryNameToGraphQlLabel(repository)}: repository(owner: "${repository.split('/')[0]}", name: "${repository.split('/')[1]}") {
      ...repositoryFragment
    }`).join('\n\n');
