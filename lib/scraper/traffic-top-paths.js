        ...
        .then((response) => {
          ...
          response.data.forEach((path) => {
            this.metrics.githubRepoPopularContentViewsGauge.set(
              { owner, repository, path: path.path }, path.count,
            );
            this.metrics.githubRepoPopularContentViewsUniqueGauge.set(
              { owner, repository, path: path.path }, path.uniques,
            );
          });
        })
        .catch((err) => logger.error(`Failed to scrape path metrics for repository ${repository} via REST: ${err.message}`, err));
    });
  }
}
