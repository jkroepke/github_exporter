        ...
        .then((response) => {
          ...
          response.data.forEach((referrer) => {
            this.metrics.githubRepoReferrerViewsGauge.set(
              { owner, repository, referrer: referrer.referrer }, referrer.count
            )
            this.metrics.githubRepoReferrerViewsUniqueGauge.set(
              { owner, repository, referrer: referrer.referrer }, referrer.uniques
            )
          })
        })
        .catch((err) => logger.error(`Failed to scrape referrer metrics for repository ${repository} via REST: ${err.message}`, err))
    })
  }
}
