...
describe('helpers', function () {
  describe('#splitArray()', function () {
    it('splitt array into chunks', function () {
      assert.deepEqual(helpers.splitArray([1, 2, 3], 2), [[1, 2], [3]]);
    });
  });

  describe('#transformRepositoryNameToGraphQlLabel()', function () {
    it('transform repo', function () {
      assert.equal(helpers.transformRepositoryNameToGraphQlLabel('owner/repo'), 'owner_repo');
    });

    it('transform hypen', function () {
      assert.equal(helpers.transformRepositoryNameToGraphQlLabel('owner/re-po'), 'owner_re_po');
    });
