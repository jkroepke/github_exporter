...
describe("helpers", function () {
  ...
  describe("#transformRepositoryNameToGraphQlLabel()", function () {
    ...
    it("transform hypen", function () {
      assert.strictEqual(
        helpers.transformRepositoryNameToGraphQlLabel("owner/re-po"),
        "owner_re_po",
      );
    });
  ...
  describe("#getWeekNumber()", function () {
    it("same week today", function () {
      assert.strictEqual(helpers.getWeekNumber(new Date("2020-02-10")), 7);
    });

    it("week end of year", function () {
      assert.strictEqual(helpers.getWeekNumber(new Date("2020-01-01")), 1);
    });
